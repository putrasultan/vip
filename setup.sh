...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Putra Sultan VPN Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}Putra SultanVPN ${NC}${YELLOW}(${NC} ${green}KURNIAWAN SETIADI ${NC}${YELLOW})${NC}"
echo -e "       © Recode By Putra Sultan VPN ${YELLOW}(${NC} 2024 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
###### IZIN SC 
...
data_ip="https://raw.githubusercontent.com/putrasultan/vip/main/izin"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/Putra_Sultan"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/6282214143867"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    echo -e ""
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/putrasultan/vip/main/izin | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/putrasultan/vip/main/izin | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/putrasultan/vip/main/izin | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/PutraSultan/vip/main/"
...
function password_default() {
	...
	today=`date -d "0 days" +"%Y-%m-%d"`
	valid=$(curl https://raw.githubusercontent.com/putrasultan/vip/main/izin | grep $MYIP | awk '{print $3}')
	d1=$(date -d "$valid" +%s)
	d2=$(date -d "$today" +%s)
	certifacate=$(((d1 - d2) / 86400))
    userdel jame > /dev/null 2>&1
    Username=$(curl https://raw.githubusercontent.com/putrasultan/vip/main/izin | grep $MYIP | awk '{print $2}')
    Password=Putra Sultan
    mkdir -p /home/script/
    ...
    TEXT="Installasi VPN Script Putra Sultan
	◇━━━━━━━━━━━━━━━━━◇
...
wget -O /usr/bin/ws "https://raw.githubusercontent.com/Diah082/newbie/main/Fls/ws" >/dev/null 2>&1

wget -O /usr/bin/tun.conf "https://raw.githubusercontent.com/Diah082/newbie/main/Cfg/tun.conf" >/dev/null 2>&1

wget -O /etc/systemd/system/ws.service "https://raw.githubusercontent.com/Diah082/newbie/main/Fls/ws.service" >/dev/null 2>&1
...
print_success "UDP-CUSTOM BY Putra Sultan"
}
...
cat >/etc/cron.d/log_clear <<-END
		0 */6 * * * root /usr/local/bin/log_clear
	END
...
echo ""
clear
figlet  Putra Sultan Store  | lolcat
echo " "
echo "=====================-[ Putra Sultan VPN ]-===================="
echo ""
echo "------------------------------------------------------------"
echo ""
echo ""
